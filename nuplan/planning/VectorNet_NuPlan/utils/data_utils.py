...
def decoding_features(observations):
    ...
    for i in range(config.NUM_AGENTS + 1):
        ...
        agent_end_x = agent_data[:, 3]
        agent_end_y = agent_data[:, 4]
...
def increment_to_trajectories(predictions, agent_current_pose_list):
    increments = predictions.view(1, config.NUM_FUTURE_POSES, 2)
    predicted_path_list = torch.zeros(1, config.NUM_FUTURE_POSES, 2)
    predicted_path_list[:, 0, :] = agent_current_pose_list[0] + increments[:, 0, :]
...
def draw_prediction(ego_trajectory, agent_trajectories, alpha=1, linewidth=1, color='orange'):
    plt.plot(
        ego_trajectory[:, 0], 
        ego_trajectory[:, 1], 
        color=color, 
...
def draw_trajectory(ego_trajectory, agent_trajectories, alpha=1, linewidth=1):
    # plot ego 
    plt.plot(
        ego_trajectory[:, 0], 
        ego_trajectory[:, 1], 
